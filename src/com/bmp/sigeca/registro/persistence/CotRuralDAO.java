 ...
 */
public class CotRuralDAO extends GenericDAO {

    /** Crea una nueva instancia de CotRuralDAO */
    public CotRuralDAO(TransactionContext context) {
        super(context);
    ...
    public long guardarCotRural(HashMap map) throws DAOException{
        ...
        try {
            ...
            pk = (Long)tablaDAO.getNextPK("codCotRural","TCOT_RURAL");

            StringBuffer sql = new StringBuffer();
            sql.append("INSERT INTO TCOT_RURAL(codCotRural,numFicha,apePaterno,apeMaterno,nombre,codTipDocumento,numDocumento,codEstCivil) ");
            sql.append("VALUES(?,?,?,?,?,?,?,?)");
    ...
    public long actualizarCotRural(HashMap map) throws DAOException{
        ...
        try {
            StringBuffer sql = new StringBuffer();
            sql.append("UPDATE TCOT_RURAL SET apePaterno=?,apeMaterno=?,nombre=?,codTipDocumento=?,numDocumento=?,codEstCivil=? ");
    ...
    public int eliminarCotRural(HashMap map) throws DAOException{
        ...
        try{
            StringBuffer sql = new StringBuffer();
            sql.append("DELETE FROM TCOT_RURAL WHERE codCotRural = ?");
    ...
    public int eliminarCotRuralByFicha(HashMap map) throws DAOException{
        PreparedStatement pstm = null;
        int result = 0;
        try{
            StringBuffer sql = new StringBuffer();
            sql.append("DELETE FROM TCOT_RURAL WHERE numFicha = ?");
    ...
    public List obtenerListaCotRural(HashMap map) throws DAOException{
        ...
        try {
            StringBuffer sql = new StringBuffer();
            sql.append("SELECT c.codCotRural,c.numFicha,c.apePaterno,c.apeMaterno,c.nombre,c.codTipDocumento,c.numDocumento,c.codEstCivil,");
            sql.append("td.descripcion,ec.descripcion ");
            sql.append("FROM TCOT_RURAL c, TTIP_DOC_IDENTIDAD td, TEST_CIVIL ec ");
            sql.append("WHERE c.codTipDocumento=td.codTipDocIdentidad AND c.codEstCivil=ec.codEstCivil AND c.numFicha = ? ");
