...
public class PredioDAO extends GenericDAO{
    ...
    /** Crea una nueva instancia de PredioDAO */
    public PredioDAO(TransactionContext context) {
        super(context);
    ...
    public long guardarPredio(HashMap map) throws DAOException{
        ...
        try {
            ...
            pk = (Long)tablaDAO.getNextPK("codPredio","TPREDIO");

            StringBuilder sql = new StringBuilder();
            sql.append("INSERT INTO TPREDIO(numFicha,codTipEdificacion,nomEdificacion,codTipInterior,numInterior,");
            ...
            sql.append("patrimonio,denominacion) ");
            sql.append("VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,");
    ...
    public long actualizarPredio(HashMap map) throws DAOException{
        ...
        try {
            StringBuilder sql = new StringBuilder();
            sql.append("UPDATE TPREDIO SET codTipEdificacion=?,nomEdificacion=?,codTipInterior=?,numInterior=?,codHabUrbana=?,nomHabUrbana=?,");
            ...
            sql.append("WHERE codPredio=? AND numFicha=? ");
    ...
    public int eliminarPredio(HashMap map) throws DAOException{
        PreparedStatement pstm = null;
        int result = 0;
        try{
            StringBuilder sql = new StringBuilder();
            sql.append("DELETE FROM TPREDIO WHERE numFicha = ? ");
    ...
    public HashMap obtenerPredio(HashMap map) throws DAOException{
        ...
        try {
            StringBuilder sql = new StringBuilder();
            sql.append("SELECT DISTINCT f.id_ficha,f.nro_ficha,e.tip_edificacion,e.nom_edificacion,u.tip_interior,u.nro_interior, ");
            sql.append("hu.cod_hab_urba,hu.nom_hab_urba,hu.grupo_urba,sd.mzna_muni,sd.lote_muni,sd.sub_lote_muni, ");
            sql.append("fi.clasificacion,fi.contenido_en,fi.cod_uso,l.estructuracion,l.zonificacion,fi.area_titulo, ");
            sql.append("fi.area_declarada,fi.area_verificada,li.frente_campo,li.der_campo,li.izq_campo,li.fondo_campo, ");
            sql.append("li.frente_titulo,li.der_titulo,li.izq_titulo,li.fondo_titulo,li.frente_colinda_campo, ");
            sql.append("li.der_colinda_campo,li.izq_colinda_campo,li.fondo_colinda_campo,li.frente_colinda_titulo, ");
            sql.append("li.der_colinda_titulo,li.izq_colinda_titulo,li.fondo_colinda_titulo,sb.luz,sb.agua,sb.telefono, ");
            sql.append("sb.desague,sb.nro_sum_luz,sb.nro_contrato_agua,sb.nro_telefono,t.condic_de_titular,t.forma_adquisicion, ");
            sql.append("to_char(t.fecha_adquisicion,'dd/MM/yyyy') as fecha_adquisicion,ep.condicion,ep.nro_resolucion,ep.porcentaje, ");
            sql.append("to_char(ep.fecha_inicio,'dd/MM/yyyy') as fecha_inicio,to_char(ep.fecha_vencimiento,'dd/MM/yyyy') as fecha_vencimiento, ");
            sql.append("fi.evaluacion,sd.sys_dir ");
            sql.append("FROM fichas f, edificaciones e, uni_cat u, hab_urba hu, sys_direcciones sd, fichas_individuales fi,  ");
            sql.append("lotes l, linderos li, servicios_basicos sb, titulares t, exoneraciones_predio ep ");
            sql.append("WHERE f.id_uni_cat=u.id_uni_cat AND u.id_edificacion=e.id_edificacion AND u.id_lote=l.id_lote ");
            sql.append("AND f.id_ficha=fi.id_ficha AND f.id_ficha=li.id_ficha AND f.id_ficha=sb.id_ficha AND l.id_hab_urba=hu.id_hab_urba ");
            sql.append("AND hu.id_hab_urba=sd.id_hab_urba AND t.id_ficha=f.id_ficha AND f.id_ficha=ep.id_ficha ");
            ...
            if(rst.next()){
                predio = new HashMap();
                predio.put("id_ficha", rst.getString(1));
                ...
                predio.put("codClaPredio", rst.getString(13));
                predio.put("codUbiPreCatastral", rst.getString(14));
                predio.put("codUsoPreCatastral", rst.getString(15));
                predio.put("estructuracion", rst.getString(16));
                predio.put("zonificacion", rst.getString(17));
                predio.put("areTerTitulo", rst.getString(18));
                predio.put("areTerDeclarada", rst.getString(19));
                predio.put("areTerVerificada", rst.getString(20));
                predio.put("medCamFrente", rst.getString(21));
                predio.put("medCamDerecha", rst.getString(22));
                predio.put("medCamIzquierda", rst.getString(23));
                predio.put("medCamFondo", rst.getString(24));
                predio.put("medTitFrente", rst.getString(25));
                predio.put("medTitDerecha", rst.getString(26));
                predio.put("medTitIzquierda", rst.getString(27));
                predio.put("medTitFondo", rst.getString(28));
                predio.put("colCamFrente", rst.getString(29));
                predio.put("colCamDerecha", rst.getString(30));
                predio.put("colCamIzquierda", rst.getString(31));
                predio.put("colCamFondo", rst.getString(32));
                predio.put("colTitFrente", rst.getString(33));
                predio.put("colTitDerecha", rst.getString(34));
                predio.put("colTitIzquierda", rst.getString(35));
                predio.put("colTitFondo", rst.getString(36));
                if(Properties.Si.equals(rst.getString(37))){
                    predio.put("luz", Properties.On);
                }else{
                    predio.put("luz", Properties.Off);
                }
                if(Properties.Si.equals(rst.getString(38))){
                    predio.put("agua", Properties.On);
                }else{
                    predio.put("agua", Properties.Off);
                }
                if(Properties.Si.equals(rst.getString(39))){
                    predio.put("telefono", Properties.On);
                }else{
                    predio.put("telefono", Properties.Off);
                }
                if(Properties.Si.equals(rst.getString(40))){
                    predio.put("desague", Properties.On);
                }else{
                    predio.put("desague", Properties.Off);
                }
                predio.put("numSumLuz", rst.getString(41));
                predio.put("numConAgua", rst.getString(42));
                predio.put("numTelefono", rst.getString(43));
                predio.put("codConTitular", rst.getString(44));
                predio.put("codForAdqPredio", rst.getString(45));
                predio.put("fecAdquisicion", rst.getString(46));
                predio.put("codConEspPredio", rst.getString(47));
                predio.put("numResExoPredio", rst.getString(48));
                predio.put("porcentaje", rst.getString(49));
                predio.put("fecInicio", rst.getString(50));
                predio.put("fecVencimiento", rst.getString(51));
                predio.put("codEvaPreCatastral", rst.getString(52));
                predio.put("sys_dir", rst.getString(53));
            }
