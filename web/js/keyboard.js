      ...
      for (var y = 0, keyid = "", ex; ex = elem[y++];) {
        if ((ex.nodeName == "TEXTAREA" || ex.type == "text" || ex.type == "password") && ex.className.indexOf("keyboardInput") > -1) {
          ...
          var keybut = document.createElement('img');
              keybut.src = "imagenes/keyboard.png";
              keybut.alt = "Teclado virtual";
              keybut.className = "keyboardInputInitiator";
              keybut.title = "Teclado virtual";
          ...
          var label = document.createElement('label');
            var checkbox = document.createElement('input');
                ...
                checkbox.onclick = function() {
                  ...
                  this.nextSibling.nodeValue = " " + ((this.checked) ? "" : "");
                  self.VKI_modify("");
                  return true;
                };
              label.appendChild(checkbox);
              label.appendChild(document.createTextNode(" " + ((checkbox.checked) ? "" : "")))
        ...
        switch (lkey[1]) {
          ...
          default:
            ...
            if (this.VKI_deadkeysOn) {
              var cchar = tds[y].firstChild.nodeValue;
              if (this.VKI_dead) {
                if (cchar == this.VKI_dead) dead = true;
                for (var z = 0; z < this.VKI_deadkey[this.VKI_dead].length; z++)
                  if (cchar == this.VKI_deadkey[this.VKI_dead][z][0]) { target = true; break; }
              }
              for (key in this.VKI_deadkey) if (key === cchar) { alive = true; break; }
            }
        }
